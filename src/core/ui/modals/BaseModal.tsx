...
export default function BaseModal({
		...
		if (scrollDirection === 'both') {
			return (
				<ScrollView
					...
					keyboardShouldPersistTaps="handled"
				>
					<ScrollView horizontal showsHorizontalScrollIndicator contentContainerStyle={styles.horizontalScrollContainer} nestedScrollEnabled removeClippedSubviews keyboardShouldPersistTaps="handled">
						{body}
					</ScrollView>
				</ScrollView>
			)
			...
			<View style={[styles.footer, { borderTopColor: colors.border + '20' }]}>
				<View style={styles.buttonRow}>
					{buttons.map((button, index) => (
						<React.Fragment key={`modal-button-${index}`}>{button}</React.Fragment>
					))}
				</View>
			</View>
		)
	...
	const getModalStyle = (): any => {
		const baseStyle = { backgroundColor: colors.background, borderWidth: 1, borderColor: colors.modalBorder }
	...
	if (variant === 'fullscreen') {
		return (
			<Modal visible={visible} animationType="none" transparent={false} onRequestClose={onClose} testID={testID}>
				<ModalBackdrop style={[styles.fullscreenContainer, { backgroundColor: colors.background }]} dataSet={{ baseModal: 'true' }}>
	...
	return (
		<Modal visible={visible} animationType="none" transparent onRequestClose={onClose} testID={testID}>
			<ModalBackdrop style={[styles.overlay, { backgroundColor: colors.modalOverlay }, getContainerStyle(), containerStyle]} dataSet={{ baseModal: 'true' }}>
